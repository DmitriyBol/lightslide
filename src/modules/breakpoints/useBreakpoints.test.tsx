...
describe('useBreakpoints', () => {
	...
	it('returns the base while no query matches', () => {
		installMatchMedia();
		...
		expect(result.current).toEqual({slidesPerView: 1.2});
	});

	it('merges the overrides of a matching query over the base', () => {
	...
	it('ignores overrides of non-matching queries in a multi-query record', () => {
		...
		expect(result.current).toEqual({slidesPerView: 2, gap: 8});
	});

	it('reacts live to a query flipping both ways', () => {
		const mm = installMatchMedia();
		const {result} = renderHook(() =>
			useBreakpoints({slidesPerView: 1}, {'(min-width: 768px)': {slidesPerView: 2}}),
		);
		expect(result.current).toEqual({slidesPerView: 1});
	...
	it('survives the breakpoints record being a new object every render', () => {
		...
		const {result, rerender} = renderHook(
			(spv: number) =>
				useBreakpoints(
					{slidesPerView: 1},
					{'(min-width: 768px)': {slidesPerView: spv}},
				),
			{initialProps: 2},
		);
		rerender(3);
	...
	it('falls back to the base where matchMedia is unavailable', () => {
...
describe('useBreakpoints — composed with the carousel', () => {
	const original = window.matchMedia;
	afterEach(() => {
		window.matchMedia = original;
	});

	/** ResizeObserver mock — the carousel measures on mount. */
	...
	it('applies overrides to the carousel while their query matches and reverts when it stops', () => {
		const mm = installMatchMedia();
