 ...
 * `setPointerHandlers`, which the core spreads onto the viewport while active. `containerRef`
 * is where the plugin binds its own hover/focus pause listeners (useHoverFocus lives with the
 * plugins that pause on engagement, not in the core). Lives in its own
