 ...
 * trackOffset applies in pixels. In loop mode the window wraps modulo slideCount so edge
 * windows cover the slides whose clones are on screen; loop never flush-clamps. The
 * wrap math relies on `margin < slideCount` — larger margins exit through the
 * covers-the-whole-strip branch first.
 */
export function buildMountPredicate(
	...
	if (isLoop) {
		const length = visibleCount + 2 * margin;
		if (length >= slideCount) return () => true;
		const start = (currentIndex - margin + slideCount) % slideCount;
